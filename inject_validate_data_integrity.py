...
def process_xml_file(file_name, table_name, columns, check_column, foreign_key_columns=None, foreign_key_tables=None):
    ...
    for record in root:
        ...
        if foreign_key_columns and foreign_key_tables:
            foreign_key_check = True
            for i, fk_col in enumerate(foreign_key_columns):
                fk_table = foreign_key_tables[i]
                fk_value = values[columns.index(fk_col)]
                fk_query = f"SELECT * FROM {fk_table} WHERE {fk_col} = %s"
                cursor.execute(fk_query, (fk_value,))
                fk_result = cursor.fetchone()
                if fk_result is None:
                    print(f"Error on line {line_number}: Foreign key constraint violation for {fk_col}={fk_value} in table {fk_table}")
...
process_xml_file('alim_utf8.xml', 'ALIM', ['alim_code', 'alim_nom_fr', 'ALIM_NOM_INDEX_FR', 'alim_nom_eng', 'alim_grp_code', 'alim_ssgrp_code', 'alim_ssssgrp_code'], 'alim_code', foreign_key_columns=['alim_grp_code', 'alim_ssgrp_code', 'alim_ssssgrp_code'], foreign_key_tables=['ALIM_GRP', 'ALIM_GRP', 'ALIM_GRP'])

process_xml_file('compo_utf8.xml', 'COMPO', ['alim_code', 'const_code', 'teneur', 'min', 'max', 'code_confiance', 'source_code'], 'alim_code', foreign_key_columns=['alim_code', 'const_code', 'source_code'], foreign_key_tables=['ALIM', 'CONST', 'SOURCES'])
